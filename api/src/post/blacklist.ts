...
export default async function blacklistPostHandler(req: FastifyRequest, res: FastifyReply) {
    const { ecosystem, name, version, repository, comment, reference, author } = req.body as OSVHandlerParams
    if (!name || !version || !comment || !author) {
        return res.status(400).send({ error: "Missing name, version, ecosystem, comment or author." })
    }

    try {
        console.log(`Adding to blacklist: name=${name}, version=${version}, ecosystem=${ecosystem}, repository=${repository}, comment=${comment}, reference=${reference}, author=${author}`)
