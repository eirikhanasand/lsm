...
export default async function whitelistPutHandler(req: FastifyRequest, res: FastifyReply) {
    ...
    try {
        console.log(`Replacing whitelist version: name=${name}, versions=${versions}, ecosystems=${ecosystems}, repositories=${repositories}, author=${author}, references=${references}, comment=${comment}`)
        ...
        await runInTransaction(async (client) => {
            ...
            await client.query(`UPDATE whitelist SET comment = $2 WHERE name = $1;`, [name, comment])
